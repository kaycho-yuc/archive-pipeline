...
- **클라우드 백엔드(2026-07-30 추가)**: `RAG_EMBED_PROVIDER`/`RAG_GEN_PROVIDER` 를 `gemini` 로
  두면 Ollama 없는 머신에서도 색인·답변이 된다(기본값은 `ollama`라 GPU 머신은 그대로).
  - 임베딩 `gemini-embedding-001`(3072차원). **차원이 LanceDB 스키마에 박히므로** 모델을 바꾸면
    `RAG_EMBED_DIM` 을 맞추고 `--reset` 재색인이 필요하다. 개수·차원 불일치는 색인 전에 예외.
  - **주의**: `gemini-embedding-2` 는 입력 여러 개를 줘도 오류 없이 벡터 1개만 돌려준다. 그대로
    쓰면 청크와 벡터가 어긋나 검색이 조용히 망가진다. 개수가 어긋나면 1건씩 재호출한다.
    API 배치 상한은 100건이라 그 단위로 나눠 보낸다.
  - 답변 `gemini-3.1-flash-lite`. **추론(thinking)을 반드시 제한한다** — 안 막으면 답변 하나에
    30~120초가 걸려 대화형으로 못 쓴다(실측 후 2초대로 감소). 일시적 503 은 3회 재시도.
  - `telegram_bot` 은 모델을 직접 넘기지 않는다. `rag_local` 이 백엔드에 맞는 모델을 고른다
    (Ollama 이름을 클라우드로 넘기면 없는 모델로 실패).
